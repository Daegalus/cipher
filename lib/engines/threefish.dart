library cipher.engines.threefish;

import "package:cipher/api.dart";
import "package:cipher/digests/skein.dart";
import "package:cipher/params/key_parameter.dart";
import "package:cipher/params/parameters_for_threefish.dart";
import "package:cipher/src/util.dart";
import "dart:typed_data";

class Threefish implements BlockCipher {
  static const int KEY_SCHEDULE_CONST = 0x1BD11BDAA9FC1A22;

  static const int EXPANDED_TWEAK_SIZE = 3;
  ...
  Uint64List expanedKey;

  Uint64List expanedTweak;

  Uint64List _cipherIn;

  Uint64List _cipherOut;

  Threefish() {
    expanedTweak = new Uint64List(EXPANDED_TWEAK_SIZE);
  }

  set tweak(Uint64List tweak) => _setTweak(tweak);

  void _setTweak(Uint64List tweak) {
    expanedTweak[0] = tweak[0];
    expanedTweak[1] = tweak[1];
    expanedTweak[2] = tweak[0] ^ tweak[1];
  }

  set key(Uint64List key) => setKey(key);

  void setKey(Uint64List key) {
    int i;
    int parity = KEY_SCHEDULE_CONST;
  ...
  static Threefish createCipher(int stateSize) {
    switch (stateSize) {
      case 256:
  ...
  void init(bool forEncryption, CipherParameters params) {
    ...
    if (params is ParametersForThreefish) {
      ParametersForThreefish pft = params as ParametersForThreefish;

      _stateSize = pft.stateSize;
      _setCipher(_stateSize);
      if (_cipher == null)throw new ArgumentError("Threefish: unsupported state size: " + _stateSize.toString());

      Uint8List key = ((pft.parameters as KeyParameter)).key;
      if (key.length != (_stateSize ~/ 8))throw new ArgumentError("Threefish: key length does not match state size: " + key.length.toString());

      Uint64List tweak = pft.tweak;
      if (tweak == null)throw new ArgumentError("Threefish: tweak data not set");
      _cipher.tweak = tweak;

      // Get a long array for cipher key and moves the byte key buffer to it
      Uint64List keyLong = new Uint64List(_stateSize ~/ 64);
      for (int i = 0; i < keyLong.length; i++)
        keyLong[i] = ByteLong.GetUInt64(key, i * 8);

      _cipher.setKey(keyLong);

      _forEncryption = forEncryption;

      // Allocate buffers
      _cipherIn = new Uint64List(_stateSize ~/ 64);
      _cipherOut = new Uint64List(_stateSize ~/ 64);

      return;
    }
    throw new ArgumentError("Threfish: invalid parameter passed to init.");
  }

  String get algorithmName => "Threefish";
  int get blockSize => _stateSize ~/ 8;

  int processBlock(Uint8List inp, int inpOff, Uint8List out, int outOff) {

    int blockLenByte = _stateSize ~/ 8;
    int blockLenLong = _stateSize ~/ 64;

    if (_cipher.expanedKey == null)
    {
      throw new Exception("Threefish: engine not initialised");
    }

    if ((inpOff + blockLenByte) > inp.length)
    {
      throw new Exception("Threefish: input buffer too short");
    }

    if ((outOff + blockLenByte) > out.length)
    {
      throw new Exception("Threefish: output buffer too short");
    }
    for (int i = 0; i < blockLenLong; i++)
      _cipherIn[i] = ByteLong.GetUInt64(inp, inpOff + i * 8);

    if (_forEncryption)
    {
      _cipher.encrypt(_cipherIn, _cipherOut);
    }
    else
    {
      _cipher.decrypt(_cipherIn, _cipherOut);
    }
    ByteLong.PutBytes(_cipherOut, out, outOff, blockLenByte);
  ...
  void encrypt(Uint64List input, Uint64List output) {}

  void decrypt(Uint64List input, Uint64List output) {}
}

class Threefish256 extends Threefish {

  static const int _CIPHER_SIZE = 256;
  static const int _CIPHER_QWORDS = _CIPHER_SIZE ~/ 64;
  static const int _EXPANDED_KEY_SIZE = _CIPHER_QWORDS + 1;

  Threefish256() {
    expanedKey = new Uint64List(_EXPANDED_KEY_SIZE);
    expanedKey[_EXPANDED_KEY_SIZE - 1] = Threefish.KEY_SCHEDULE_CONST;
  }

  void encrypt(Uint64List input, Uint64List output) {

    int b0 = input[0], b1 = input[1],
    b2 = input[2], b3 = input[3];
    int k0 = expanedKey[0], k1 = expanedKey[1],
    k2 = expanedKey[2], k3 = expanedKey[3],
    k4 = expanedKey[4];
    int t0 = expanedTweak[0], t1 = expanedTweak[1],
    t2 = expanedTweak[2];
...
class Threefish512 extends Threefish {

  static const int _CIPHER_SIZE = 512;
  static const int _CIPHER_QWORDS = _CIPHER_SIZE ~/ 64;
  static const int _EXPANDED_KEY_SIZE = _CIPHER_QWORDS + 1;
...
class Threefish1024 extends Threefish {
  static const int _CIPHER_SIZE = 1024;
  static const int _CIPHER_QWORDS = _CIPHER_SIZE ~/ 64;
  static const int _EXPANDED_KEY_SIZE = _CIPHER_QWORDS + 1;
